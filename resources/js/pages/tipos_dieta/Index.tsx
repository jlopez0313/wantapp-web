...
export default function Index({ auth, lista }: any) {
    ...
    const onTrash = async (id: number) => {
        ...
        if (result.isConfirmed) {
            router.delete(route('tipos_dieta.destroy', id), {
                preserveScroll: true,
                onSuccess: async () => {
                    ...
                    if (remainingItems === 0 && currentPage > 1) {
                        router.visit(`/tipos_dieta?page=${currentPage - 1}`);
                    } else {
                        router.visit(`tipos_dieta?page=${currentPage}`);
                    }
                },
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <Modal show={show} closeable={true} title="Gestionar Tipos de Dieta">
